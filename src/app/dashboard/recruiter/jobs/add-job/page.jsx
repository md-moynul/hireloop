      ...
      <Card className="w-full mb-8 p-4 bg-default-50/50" shadow="sm">
        <Card.Header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <Card.Title className="text-base font-semibold">Posting as: {companyData.name}</Card.Title>
              <Card.Description className="text-xs">
                Plan Level: <span className="text-primary font-semibold">{companyData.plan}</span> ({companyData.activeJobsCount}/{companyData.jobLimit} active slots used)
              </Card.Description>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {isLimitReached && companyData.isApproved && (
